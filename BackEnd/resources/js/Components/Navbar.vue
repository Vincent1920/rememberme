    ...
    <nav class="bg-yellow-50 shadow px-6 py-4 flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center space-x-6">
            ...
            <template v-if="!isAuthenticated">
                <router-link to="/login" class="font-bold text-gray-800 hover:text-yellow-600 transition">
                    Login
                </router-link>
            </template>
            ...
            <template v-else>
                ...
                <router-link to="/kategori" class="hover:text-black">Kategori</router-link>
    ...
    <div v-if="menuOpen" class="md:hidden bg-yellow-50 w-full px-6 py-4 space-y-4 text-gray-800 shadow">
        ...
        <template v-if="!isAuthenticated">
             ...
             <router-link to="/login"
                class="block text-center py-2 bg-gray-900 text-white rounded hover:bg-black font-bold">
                Login
            </router-link>
        </template>
        ...
        <template v-else>
            ...
            <div class="mt-4 p-3 bg-white rounded shadow space-y-2">
                <p class="font-semibold text-gray-700">Nama: {{ name }}</p>
                <p class="text-sm text-gray-500">Email: {{ email }}</p>

                <button @click="logout" class="mt-3 w-full text-left text-red-600 hover:text-red-700 font-medium">
                    Logout
                </button>
            </div>
        </template>
